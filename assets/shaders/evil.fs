...
vec4 dissolve_mask(vec4 tex, vec2 texture_coords, vec2 uv) // leave alone
{
    if (dissolve < 0.001) {
...
number hue(number s, number t, number h) // leave alone
{
    number hs = mod(h, 1.)*6.;
...
#ifdef VERTEX
vec4 position( mat4 transform_projection, vec4 vertex_position ) // leave alone
{
    if (hovering <= 0.){
